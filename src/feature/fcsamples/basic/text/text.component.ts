...
export class TextComponent extends ComponentParent {
  ...
  `
  //后缀事件view
  addonafterview: string = `<fc-text [fcLabel]="'后置图标事件'" [(ngModel)]="addonbefore" fcAddOnAfterIcon="icon iconfont fc-icon-unit" (fcEvent)="addonafterEvent($event)"></fc-text>
  `
  //后缀事件code
  ...
  <fc-text [fcLabel]="'搜索事件'"  [(ngModel)]="addonbefore"fcSearch="Y" fcSuffixButton="search" (fcEvent)="textEvent($event)"></fc-text>
  `
  //查询事件code
  textEvenjs: string = `
  import { Component, OnInit } from '@angular/core';
