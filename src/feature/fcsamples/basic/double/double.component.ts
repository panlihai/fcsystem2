...
export class DoubleComponent extends ComponentParent {
  ...
  `
   ...
   //只读
   readonlyview: string = `
   <fc-date [fcLabel]="'只读'" [(ngModel)]="now" fcReadonly="Y"></fc-date>
   `
   //禁用
  disabledview: string = `
  <fc-date [fcLabel]="'禁用'" [(ngModel)]="now"  fcDisabled="Y"></fc-date>
  `
   // 基本basicjs
   basicjs: string = `
   import { Component, OnInit } from '@angular/core';
  ...
  setReadOnly(event){
    this.readonly=this.readonly==='Y'?'N':'Y';
  }
  setEnabled(event){
    this.disabled=this.disabled==='Y'?'N':'Y';
  }
}
