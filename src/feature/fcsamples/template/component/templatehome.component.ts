...
import { TimelineOptions, FclistdataComponent } from 'fccomponent2';   
      ...
      .subscribe(result => {
        if (result.CODE === "0") {
          ...
          result.DATA.forEach(item => {
            ...
            version.push({
              label: t,
              title: FCCONFIG.pid + "发布" + item.LASTVERSION + "版",
              ID: item.ID,
              color: "normal"
            });
          });
    ...
    switch (event.eventName) {
      case "selected": //选中
        CommonService.event('selectedMenu', {//跳转到版本控制详情界面
          ...
          PID: FCCONFIG.pid, MENUTYPE: 'INURL', MENUNAME: '版本控制', MENUICON: 'fc-icon-bgefficiency'
        });
        break;
    }
  }
