...
export class SysvieweditComponent extends ParentEditComponent {
    ...
    */
    handleRouterParam(): void {
        if (this.routerParam.ID) {
            SystemBusiness.appService.findWithQuery("SYSVIEW", { WHERE: "{ID:{eq:'" + this.routerParam.ID + "'}}" }).subscribe(res => {
        ...
        if (this.routerParam.funcId) {
            SystemBusiness.appService.findWithQuery('SYSFUNC', { WHERE: "{ID:{eq:'" + this.routerParam.funcId + "'}}" }).subscribe(res => {
                if (res.CODE === '0' && res.DATA.length !== 0) {
                    for (let attr in res.DATA[0]) {
                        if (attr === 'FUNCID') {
                            this.mainObj[attr] = res.DATA[0][attr];
                    ...
                    SystemBusiness.msgService.error('功能信息获取失败');
                }
            })
        }
    }
    ...
    //     SysviewBusiness.saveField(param);
    // }
    backToFuncEdit(): void {
        this.navigate(this.getUrl(this.moduleId, 'SYSFUNC', 'Edit'), { ID: this.routerParam.funcId, refresh: 'Y' });
    }
    /**
     * 初始化mainObj的默认值
     */
  ...
  */
    ...
    initSysApps(): void {
        SystemBusiness.appService.findWithQuery("SYSAPP", {}).subscribe(res => {
            if (res.CODE === '0') {
    ...
    handleSelectApp(param: any): void {
        this.selectedApps = param;
        this.selectedApps.forEach((el: any, index) => {
            SystemBusiness.appService.findAppByAid(el.APPID).subscribe(res => {
                if (res.CODE === '0') {
                    var sApp = res.DATA;
