...
export class SysmessagedetailComponent extends ParentDetailComponent {
  ...
  init(): void {
    //初始化时间轴的消息
    SysmessageBusiness.getMessageBy({}).subscribe(result => {
      if (result.CODE === '0') {
    ...
    //初始化消息详情
    SysmessageBusiness.appService.initObjDefaultValue(this.mainApp)
      .subscribe(result => {
        if (result.CODE === '0') {
  ...
  postFeedback():void {
    ...
    SysmessageBusiness.feedBack(this.feedBackObj, this.mainObj).subscribe(result => {
      if (result.CODE === '0') {
   ...
   */
  isreadCondition() {
    SysmessageBusiness.getMessageBy({ ISREAD: 'Y' }).subscribe(result => {
      if (result.CODE === '0') {
        this.timelineOption.fcValues = result.DATA;
        this.timelineOption.fcValues.forEach(element => {
          if (element.TS !== null && element !== '') {
   ...
   */
  noreadCondition() {
    SysmessageBusiness.getMessageBy({ ISREAD: 'N' }).subscribe(result => {
      if (result.CODE === '0') {
        this.timelineOption.fcValues = result.DATA;
        this.timelineOption.fcValues.forEach(element => {
          if (element.TS !== null && element !== '') {
